        ...
        if short_url:
            try:
                ...
                if url_mapping.expired_at and url_mapping.expired_at <= timezone.now():
                    return Response({"success":False,"message":"Short URL has expired"},status=status.HTTP_410_GONE)
                ...
                return Response({"success":False,"message":"Short URL not found"},status=status.HTTP_404_NOT_FOUND)
        else:
